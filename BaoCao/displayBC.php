...
if (isset($mysqli)) {
    $sql = "SELECT MaBaoCao, SanPham, TonDau, MuaVao, BanRa, TonCuoi FROM ctbaocaokho"; // Adjust table and column names as per your database structure
    ...
    if ($result) {
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc())
             {
                $id = htmlspecialchars($row["MaBaoCao"]);
